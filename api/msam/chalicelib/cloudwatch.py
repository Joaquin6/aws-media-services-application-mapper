...
def get_cloudwatch_events_state_groups(state):
    ...
    idle_nodes = channels.get_channel_state_list('idle')
    running_nodes = channels.get_channel_state_list('running')
    for event in events:
        arn = event["resource_arn"]
        pl = event["detail"]["pipeline"]
        def is_same_pl(i):
            return bool(i["resource_arn"] == arn and "pipeline" in i["detail"] and i["detail"]["pipeline"] == pl)
        def is_diff_pl(i):
            return bool(i["resource_arn"] == arn and "pipeline" in i["detail"] and i["detail"]["pipeline"] != pl)
        def is_pl_down(i):
            return bool("pipeline_state" in i["detail"] and not i["detail"]["pipeline_state"])
        idle_contents = find_idle_events(idle_nodes, event)
        running_contents = find_running_events(running_nodes, event)
        all_down_pipelines = list(filter(is_pl_down, events))
        ...
        if len(idle_contents) > 0:
        ...
        elif len(running_contents) > 0 and len(all_down_pipelines) == 0:
            event["detail"]["degraded"] = bool(False)
            event["detail"]["idle_state"] = bool(False)
            ...
            event["detail"]["degraded"] = bool(False)
            event["detail"]["idle_state"] = bool(False)
            group["undetermine"].append(event)
    return group


def find_idle_events(nodes, event):
    items = []
    arn = event["resource_arn"]
    for i in nodes:
        if "Arn" in i and i["Arn"] == arn and "State" in i and i["State"] == "IDLE":
            items.append(i)
        elif "data" in i:
            data = json.loads(i["data"])
            if "arn" in i and i["arn"] == arn:
                if "State" in data and data["State"] == "IDLE":
                    items.append(i)
                elif "idle_state" in data and data["idle_state"]:
                    items.append(i)
    return items


def find_running_events(nodes, event):
    items = []
    arn = event["resource_arn"]
    for i in nodes:
        if "Arn" in i and i["Arn"] == arn and "State" in i and i["State"] == "RUNNING":
            items.append(i)
        elif "data" in i:
            data = json.loads(i["data"])
            if "arn" in i and i["arn"] == arn:
                if "State" in data and data["State"] == "RUNNING":
                    items.append(i)
                elif "idle_state" in data and not data["idle_state"]:
                    items.append(i)
    return items
